...
class Signup
{
    ...
    public function checkEmail()
    {
        ...
        try {
            ...
            // Respond with JSON (true if available, false if taken)
            header('Content-Type: application/json');
            echo json_encode($result['available']);
            exit;
        } catch (Exception $e) {
            header('Content-Type: application/json');
	...
	public function checkContact()
    {
        ...
        $data = json_decode(file_get_contents("php://input"), true);
        if (!$data || empty($data["contact"])) {
            header('Content-Type: application/json');
            echo json_encode(["error" => "Phone number is required"]);
            exit;
        }
    
        $model = $this->model('signup', 'Check_Contact');
        if (!$model) {
            header('Content-Type: application/json');
            echo json_encode(["error" => "Model load failure"]);
            exit;
        ...
        try {
            $result = $model->check($data["contact"]);
            header('Content-Type: application/json');
            echo json_encode($result['available']);
            exit;
        } catch (Exception $e) {
